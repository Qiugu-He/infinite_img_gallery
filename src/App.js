...
export default function App() {
  ...
  useEffect(()=>{
    getPhotos();
  }, []);  // as soon as pages, this effect getPhotos run again 
  ...
  function searchPhotos(e) {
    e.preventDefault(); // as html form automatically refresh browser, when submit, so need to prevent event
    setPage(1);
    getPhotos();
  }

  //return an erro if no access key for the images
  if (!accessKey) {
    return (
